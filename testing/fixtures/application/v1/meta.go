...
const (
	Kind      = "application.testing.rxp"
	Namescope = types.NamescopeDomain
)
...
// version of the Application meta in the v1 major version series.
...
func init() {
	...
	if err != nil {
		log.Fatalf(
			"failed to construct jsonschema.Schema for Application_V1_0_0: %s",
			err.Error(),
		)
	}
	...
	if err != nil {
		log.Fatalf(
			"failed to construct jsonschema.Schema for Application_V1_0_1: %s",
			err.Error(),
		)
	}
	jsonb, err = js.MarshalJSON()
