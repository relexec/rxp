...
const (
	KindName  = types.KindName("author.testing.rxp")
...
func init() {
	...
	if err != nil {
		log.Fatalf(
			"failed to construct jsonschema.Schema for Author_V1_0_0: %s",
			err.Error(),
		)
	}
	...
	if err != nil {
		log.Fatalf(
			"failed to marshal JSON for schema for Author_V1_0_0: %s",
			err.Error(),
		)
	}
	...
	if err != nil {
		log.Fatalf(
			"failed to construct jsonschema.Schema for Author_V1_0_1: %s",
			err.Error(),
		)
	}
	jsonb, err = js.MarshalJSON()
	if err != nil {
		log.Fatalf(
			"failed to marshal JSON for schema for Author_V1_0_1: %s",
			err.Error(),
		)
	}
